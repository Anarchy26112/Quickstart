...
public class Shooter {
    ...
    public Shooter(HardwareMap hardwareMap, Telemetry telemetry) {
        ...
        // Cache a single voltage sensor to avoid extra iterator work / overhead
    ...
    public void update() {
        ...
        // 2. Poll voltage on a timer
        if (voltageTimer.seconds() >= VOLTAGE_POLL_INTERVAL_SEC) {
            refreshVoltage();
            voltageTimer.reset();
        }

        // 3. Compute and apply power
    ...
    public void stop() {
        ...
        // Let calculateAndSetPower() handle zeroing so write cache stays synced
    }
    ...
    private void calculateAndSetPower() {
        if (targetRVelocity == 0.0 && targetLVelocity == 0.0) {
            ...
            if (Math.abs(lastWrittenRPower - 0.0) > WRITE_TOLERANCE) {
                rightShooter.setPower(0.0);
                lastWrittenRPower = 0.0;
            }

            if (Math.abs(lastWrittenLPower - 0.0) > WRITE_TOLERANCE) {
                leftShooter.setPower(0.0);
                lastWrittenLPower = 0.0;
            }
            return;
        }
        ...
        // Proportional correction using cached velocities
        ...
        double newRPower = clamp((ffR + pR) * voltageCompensationRatio, -1.0, 1.0);
        double newLPower = clamp((ffL + pL) * voltageCompensationRatio, -1.0, 1.0);

        currentRPower = newRPower;
        currentLPower = newLPower;

        // Cached hardware writes
        if (Math.abs(lastWrittenRPower - newRPower) > WRITE_TOLERANCE) {
            rightShooter.setPower(newRPower);
            lastWrittenRPower = newRPower;
        }

        if (Math.abs(lastWrittenLPower - newLPower) > WRITE_TOLERANCE) {
            leftShooter.setPower(newLPower);
            lastWrittenLPower = newLPower;
        }
    }
