...
public class SpinDex {
    ...
    // --- TUNING ---
    private static final double POSITION_TOLERANCE_TICKS = 3.0;  // deadband around target
    private static final double MAX_POWER = 1.0;
    ...
    public void periodic() {
        ...
        // Update logical index for shortest-path math
        currentPositionIndex = (int) Math.round(currentTicks / TICKS_PER_POSITION);
        ...
        if (Math.abs(error) <= POSITION_TOLERANCE_TICKS) {
            ...
            // Optional: switch back to RUN_USING_ENCODER so it doesn't keep "trying"
            // (BRAKE will hold well mechanically)
        ...
        // If we have a meaningful error but motor isn't in RUN_TO_POSITION, put it there
        ...
        if (pwr < SPINDEX_MIN_POWER) pwr = SPINDEX_MIN_POWER;
